...
namespace Cinemachine
{
    ...
    /// </summary>
    public class CameraClickAndDrag : MonoBehaviour, AxisState.IInputAxisProvider
    {
        /// <summary>
        ...
        public virtual float GetAxisValue(int axis)
        {
            if (enabled)
            {
                ...
                if (action != null)
                {
                    switch (axis)
                    {
                        case 0: return Pointer.current.press.isPressed ? action.ReadValue<Vector2>().x : 0;
                        case 1: return Pointer.current.press.isPressed ? -action.ReadValue<Vector2>().y : 0;
                        case 2: return Pointer.current.press.isPressed ? action.ReadValue<float>() : 0;
                    }
                }
            }
            return 0;
